...
for (i in 2:how_many){
  depth[i]<-depth[1]-i*delta_depth# uniform depths
  print(depth[i])
  } 
...
for (i in 1:(how_many-1)){
    ...
    print((depth[i]))# + (velocity[i]^2)/(2*9.8)))
    print((depth[i+1]))#+(velocity[i+1]^1)/(1)))
...
# output
plot(distance,depth,col="blue",type="l",lwd=8,xlim=c(0,max(distance)),ylim=c(min(bse),max(wse)));
...
##zz<-backwater(begin_depth=0.1,end_depth=0.47,how_many=20,discharge=1,width=51,mannings_n=0.01,slope=0.000)
